...
public class RotatingPillar : MonoBehaviour, IInteractable
{
    ...
    {
        cropKey.OnCropInserted += OnCropInserted;
    }

    private void OnCropInserted(CropKey cropKey)
    {
        Debug.Log($"Crop inserted for {cropKey.name}. Unlocking pillar rotation.");
        cropInserted = true;
    }

    public void Interact(PlayerInteraction interactor, out bool interactSuccessful)
    {
        if (!coroutineRunning && !isLocked && cropInserted)
        {
