...
public class Boulder : StructureBehaviorScript
{
    ...
    {
        if(TimeManager.Instance.currentHour == 8 && Random.Range(0,130) == 1 && StructureManager.Instance.ValidateGridType(transform.position, GridType.Farm)) //Turn into a plugged geyser
        {
            rockNum = 3;
            UpdateModel();
        }
